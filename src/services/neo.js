...
//* 192 
//const SCE_ASSET_ID = '0x19fc3b536c1a3654fcc49e88990258888e9abe0e0df5f8100e54387e3736d745';
//const CBK_ASSET_ID = '0x09042c45210e406c38750b63a6dccee6042593114b89c1722465d4ac6ae8a432';

//* Ocean
const SCE_ASSET_ID = '0xa399858eefe404aff285d5b3ad696fbd0255a4b131d0d47df82842041f844459';
const CBK_ASSET_ID = '0x0712838683620683e1fbe87d2934269c1d60d216a073fd35e50267bfccfc74f7';
        ...
        return rpcClient.getRawTransaction(hash, 1)
          .then((transaction) => {
            ...
            transaction.vout.forEach((output) => {
              ...
              if (output.asset === SCE_ASSET_ID) {
                output.symbol = 'SCE';
              } else if (output.asset === CBK_ASSET_ID) {
                output.symbol = 'CBK';
              }
            });
            ...
            transaction.vin.forEach((input) => {
              inputPromises.push(rpcClient
                ...
                .then((inputTransaction) => {
                  ...
                  if (inputSource.asset === SCE_ASSET_ID) {
                    input.symbol = 'SCE';
                  } else if (inputSource.asset === CBK_ASSET_ID) {
                    input.symbol = 'CBK';
                  }
          ...
          .catch((e) => {
            reject(new Error(`CROWDBANK RPC Network Error: ${e.message}`));
          });
      } catch (e) {
        return reject(e);
      }
        ...
        return rpcClient.query({ method: 'getaccountstate', params: [address] })
          .then((res) => {
            ...
            res.result.balances.forEach((b) => {
              ...
              if (b.asset === CBK_ASSET_ID) { h.symbol = 'CBK'; h.name = 'CBK'; }
              if (b.asset === SCE_ASSET_ID) { h.symbol = 'SCE'; h.name = 'SCE'; }
 ...
 */
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (isNep5 === false) {
          console.log(assetId);
          console.log(amount);
          sendPromise = this.sendSystemAsset(toAddress, assetId, amount);
        } else if (isNep5 === true) {
  ...
  sendSystemAsset(toAddress, assetId, assetAmount) {
    const currentNetwork = network.getSelectedNetwork();
    ...
    if (assetAmount > 0) {
      if (assetId === NEO_ASSET_ID) intentAmounts.NEO = assetAmount;
      if (assetId === GAS_ASSET_ID) intentAmounts.GAS = assetAmount;
      if (assetId === SCE_ASSET_ID) intentAmounts.SCE = assetAmount;
      if (assetId === CBK_ASSET_ID) intentAmounts.CBK = assetAmount;
    }

    rpcClient.query({ method: 'sendfrom', params: [assetId.replace('0x', ''), currentWallet.address, toAddress, assetAmount] });
    // rpcClient.query({ method: 'sendtoaddress', params: [assetId.replace('0x', ''), toAddress,assetAmount]});
    // rpcClient.query({ method: 'dumpprivkey', params: ["AGMGEkA3SLzDeLRbvDmhsdCUzHgARbiszT"]});
    // rpcClient.query({ method: 'getnewaddress', params: []});
    // rpcClient.query({ method: 'getbalance', params: [assetId.replace('0x', '')]});
    // rpcClient.query({ method: 'getaccountstate', params: [currentWallet.address]});
    // rpcClient.query({ method: 'validateaddress', params: [currentWallet.address]});
